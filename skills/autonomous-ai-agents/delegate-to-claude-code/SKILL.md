...
- For multi-artifact builds (scripts + repo scaffolds + cron/job setup), use the JSON-output contract and verification pattern in `references/multi-artifact-delegation-20260612.md`: exact paths, commands run, counts, validation result, blockers, next steps; then re-run validators locally before finalizing.