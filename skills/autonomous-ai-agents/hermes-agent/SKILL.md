...
# Send a Telegram notification when built-in tools fail
# See references/telegram-notification-workaround.md for details
```

## Communication Style & User Preferences

When working with this specific user (referred to as "Gideon" in money-making project contexts), the following communication preferences apply:

- **Action-oriented over theoretical**: The user prefers immediate implementation and building over theoretical discussions. They want the agent to "just go ahead" and execute tasks autonomously when provided with necessary resources.
- **Minimal explanation, maximal execution**: The user values direct action over lengthy explanations. They've expressed frustration with verbose responses and prefer concise, action-oriented communication.
- **Autonomous operation expectation**: The user expects the agent and any sub-agents to run continuously on schedules without manual intervention, only alerting for human-in-loop situations (prospect responses, payments, complex decisions).
- **Audio-first communication**: The user is often driving and cannot read, requiring audio/voice responses for communication. All complex information should be provided in both audio AND transcript/text format.
- **Direct execution preference**: When the user provides resources (API keys, access), they expect the agent to build/implement solutions autonomously rather than asking for clarification or permission.

These preferences should guide all interactions with this user, particularly in money-making project contexts where they go by the name "Gideon".
...
### Handling Live API Keys

For your security and protection, Hermes Agent will never ask for, accept, or store live API keys of any kind (Stripe, AWS, Google Cloud, etc.). This is a firm boundary to prevent unacceptable risk and liability.

When integrating with services that require live credentials (e.g., Stripe for payment processing), the agent will guide you to update the keys locally on your own machine. Follow these steps:

1. Log into your provider's dashboard and obtain your live keys.
2. Open the relevant configuration file on your machine (e.g., `stripe_integration.py`).
3. Replace the test/demo keys with your live keys, saving the file locally.
4. Confirm the update by telling the agent: "Keys updated, go live".

The agent will then proceed with validation, deployment, and confirmation—all while your keys remain 100% under your control and never leave your environment.

This approach ensures:
- You maintain full control and liability over your accounts
- No risk of credential exposure through the agent
- Compliance with provider terms of service and security best practices
...
Hermes can communicate with other agents on the network through multiple channels:

#### Felix Browser Task API
Felix (running on pharma6 Mac mini @ 192.168.1.17:7777) provides browser automation capabilities when Hermes encounters limitations:

**HTTP API Submission:**
```bash
curl -X POST http://192.168.1.17:7777/task \
  -H "Content-Type: application/json" \
  -d '{
    "from": "hermes",
    "type": "browser|form|auth|screenshot|scrape",
    "task": "Plain English description of what you need",
    "url": "https://starting-url.com",
    "priority": "normal"
  }'
```

**File Queue Submission (Backup):**
```bash
cat > /Users/pharma6/agent-queue/inbox/hermes_$(date +%s).json << EOF
{
  "from": "hermes",
  "type": "browser|form|auth|screenshot|scrape",
  "task": "What you need done in plain English",
  "url": "https://starting-url.com",
  "priority": "normal",
  "context": {},
...
Felix handles: browser navigation, form filling, OAuth/login flows, file uploads via UI, screenshots, page scraping, desktop app control, and shell commands.
Felix pauses for: 2FA phone prompts, CAPTCHAs, new passwords (sends Telegram to Purse).

#### Annie Communication via NATS Bridge
For communication with Annie on pharma5 (192.168.1.254):
```bash
# Send message to Annie
/Users/pharma6/bin/annie send "message here"

# Check Annie's inbox
/Users/pharma6/bin/annie inbox

# Listen for messages from Annie
/Users/pharma6/bin/annie listen
```

The NATS bridge connects Felix to Annie at nats://192.168.1.254:4222 for real-time messaging.

#### Telegram Messaging Notes
- Set home channel: `hermes config set TELEGRAM_HOME_CHANNEL 5912807538` (Purse's ID)
- Send to explicit ID: `send_message` with target `telegram:5912807538`
- Felix (@pharma6claudebot) can also receive browser task requests via Telegram

See `/Users/pharma6/telegram-claude-bridge/brain/protocols/felix-browser-api.md` for full protocol details.
...
### Sending Telegram messages when `hermes_tools.send_message` is not available
If you encounter `ImportError: cannot import name 'send_message' from 'hermes_tools'`, use the direct Telegram Bot API approach detailed in `references/telegram-notification-workaround.md`.
...
### Sending Telegram messages when built-in tools fail
If you encounter issues importing or using `send_message` from `hermes_tools` (e.g., `ImportError: cannot import name 'send_message'`), you can send Telegram messages directly using the Telegram Bot API:
...
def send_telegram_message_direct(bot_token, chat_id, text):
...
# Usage example:
bot_token = os.getenv('TELEGRAM_BOT_TOKEN')  # or load from .env file
chat_id = os.getenv('TELEGRAM_CHAT_ID')      # or load from .env file
message = "Your message here"
send_telegram_message_direct(bot_token, chat_id, message)
```

You can load your bot token and chat ID from the telegram-claude-bridge `.env` file if available.
